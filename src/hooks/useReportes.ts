...
export interface TicketReportRow {
  ...
  final_amount:   number | null;  // solo mano de obra / reparación
  parts_amount:   number;         // suma de ticket_items
  total_amount:   number;         // final_amount + parts_amount
  parts_summary:  string;         // "Batería ×1, Pantalla ×1"
...
export interface ReportSummary {
  ...
  total_ticket_revenue:   number;  // labor + parts combined
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          ...
          ticket_items (
            quantity, unit_price, add_to_total,
            inventory ( name )
          )
        `)
        .gte("created_at", from.toISOString())
      ...
      return ((data ?? []) as any[]).map((t) => {
        ...
        const labor  = t.final_amount ? Number(t.final_amount) : 0;

        const items = (t.ticket_items ?? []) as { quantity: number; unit_price: number; add_to_total: boolean | null; inventory: { name: string } | null }[];
        // Solo sumar piezas que SÍ se cobran aparte (add_to_total !== false).
        // Las marcadas "Ya incluida en el monto" ya forman parte de final_amount.
        const parts = items
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .select(`
          id, order_number, created_at, description, tags, device_brand, device_model,
          status, payment_status, payment_method, price,
          customers ( full_name ),
          guest_name
        `)
        .gte("created_at", from.toISOString())
      ...
      return ((data ?? []) as any[]).map((s) => ({
        ...
        tags:           s.tags ?? [],
        device_brand:   s.device_brand ?? null,
...
export function useReporteGastos(from: Date, to: Date) {
  return useQuery({
    queryKey: ["reporte-gastos", from.toISOString(), to.toISOString()],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("expenses")
        .select("id, expense_date, description, category, amount, payment_method")
...
export function useReporteSummary(from: Date, to: Date) {
  ...
  const gastos   = useReporteGastos(from, to);

  const isLoading = sales.isLoading || tickets.isLoading || servicios.isLoading || gastos.isLoading;

  const total_sales_revenue    = (sales.data ?? []).filter(s => s.payment_status === "pagado").reduce((a, s) => a + s.total_amount, 0);
  const total_ticket_revenue   = (tickets.data ?? []).filter(t => t.payment_status === "pagado").reduce((a, t) => a + t.total_amount, 0);
  const total_servicio_revenue = (servicios.data ?? []).filter(s => s.payment_status === "pagado").reduce((a, s) => a + (s.price ?? 0), 0);
  const total_revenue          = total_sales_revenue + total_ticket_revenue + total_servicio_revenue;
  const total_gastos           = (gastos.data ?? []).reduce((a, g) => a + g.amount, 0);

  const summary: ReportSummary = {
    total_sales_revenue,
    total_ticket_revenue,
    total_servicio_revenue,
    total_revenue,
    sales_count:       (sales.data ?? []).length,
    tickets_count:     (tickets.data ?? []).length,
    servicios_count:   (servicios.data ?? []).length,
    gastos_count:      (gastos.data ?? []).length,
    total_gastos,
    net_profit:        total_revenue - total_gastos,
    pending_collection:
      (sales.data ?? []).filter(s => s.payment_status === "fiado").reduce((a, s) => a + s.total_amount, 0) +
      (tickets.data ?? []).filter(t => t.payment_status === "fiado").reduce((a, t) => a + t.total_amount, 0) +
      (servicios.data ?? []).filter(s => s.payment_status === "fiado").reduce((a, s) => a + (s.price ?? 0), 0),
  };

  return { summary, isLoading, sales, tickets, servicios, gastos };
}
