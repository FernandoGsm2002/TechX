...
type ExpenseInsert = Omit<ExpenseRow, "id" | "created_at" | "updated_at" | "created_by_name" | "payment_method" | "notes"> & {
  payment_method?: string | null;
  notes?: string | null;
};
...
export function useCreateGasto() {
  ...
  return useMutation({
    mutationFn: async (values: ExpenseInsert) => {
      ...
      if (error) throw error;
      // created_by_name es campo derivado del join (solo disponible en useGastos query)
      return { ...data, created_by_name: null } as ExpenseRow;
    },
    onSuccess: (data) => {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const d = data as any;
      const cat  = d.category    ? String(d.category)                      : "Gasto";
      const desc = d.description ? ` · ${String(d.description).slice(0, 40)}` : "";
      notifyAdmins(data.organization_id, `💸 ${cat}`, `-${data.amount}${desc}`).catch(() => {});
    },
...
export function useFinanzasStats(dateRange?: DateRange) {
  return useQuery({
    queryKey: ["finanzas-stats", dateRange?.start, dateRange?.end],
    queryFn: async (): Promise<FinanzasStats> => {
      ...
      if (dateRange) {
        ...
        startOfNextMonth = dateRange.end;
      } else {
        const now   = new Date();
        const year  = now.getFullYear();
        const month = String(now.getMonth() + 1).padStart(2, "0");
        startOfMonth     = `${year}-${month}-01T00:00:00.000Z`;
        startOfNextMonth = month === "12"
      ...
      const ticketIncome = ((paidTickets ?? []) as {
        ...
        return sum + Number(t.final_amount ?? t.quote_amount ?? 0) + Number(t.parts_amount ?? 0);
      }, 0);
      ...
      const porCobrar =
        ((fiadoTickets ?? []) as { final_amount: number | null; quote_amount: number | null; parts_amount: number }[])
          .reduce((s, t) => s + Number(t.final_amount ?? t.quote_amount ?? 0) + Number(t.parts_amount ?? 0), 0) +
