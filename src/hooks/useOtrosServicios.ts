...
export function useUpdateOtroServicioStatus() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      // Read current status before updating (needed for history)
      const { data: current, error: fetchErr } = await supabase
        .from("otros_servicios")
        .select("status, organization_id, description, price")
        .eq("id", id)
        .single();
      ...
      // Registrar cambio en historial (igual que ticket_history)
      const { data: { user } } = await supabase.auth.getUser();
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (supabase as any).from("servicio_history").insert({
        servicio_id:     id,
        organization_id: (current as { organization_id: string }).organization_id,
        status_from:     (current as { status: string }).status,
        status_to:       status,
    ...
    onSuccess: (data, { id, status }) => {
      ...
      if (org?.id) {
        const name = data.description ? String(data.description).slice(0, 45) : "Servicio";
        ...
        const body = status === "entregado" && data.price
          ? `${name} entregado — +${data.price} cobrado`
          : `${name} → ${statusLabels[status] ?? status}`;
        notifyAdmins(org.id, "🛠 Servicio", body).catch(() => {});
      }
    },
  });
}
...
export function useOtrosServicios() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      // Los técnicos ven todos los servicios (a diferencia de tickets).
      // Si se decide restringir, añadir aquí: q = q.eq("assigned_to", userId);
