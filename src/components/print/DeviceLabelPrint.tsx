...
export function DeviceLabelPrint({ data, variant = "button" }: DeviceLabelPrintProps) {
  ...
  const handlePrint = () => {
    ...
    const win = window.open("", "_blank", "width=400,height=250,toolbar=0,menubar=0") as unknown as Window | null;
    if (!win) {
  ...
  const ticketShort = data.ticketNumber || shortId(data.ticketNumber); // ticketNumber always provided; shortId is fallback
