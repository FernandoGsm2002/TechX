...
import {
  CheckCircle2, XCircle, Clock, Building2, Loader2, RefreshCw,
  ShieldOff, Trash2, CreditCard, CalendarDays, ChevronDown,
  ChevronUp, Search, Globe, Phone, MapPin,
} from "lucide-react";
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            {status !== "pending" && (
              <>
                ...
                >
                  {isPending ? <Loader2 className="size-3.5 animate-spin" /> : <ShieldOff className="size-4" />}
                </Button>
                <Button
                  size="icon"
                  variant="ghost"
          ...
          <div className="px-4 pb-4 border-t border-border/30 pt-3 grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs text-muted-foreground">
            {org.address && (
              <span className="flex items-center gap-1.5">
                <MapPin className="size-3 shrink-0" /> {org.address}
              </span>
            )}
            {org.phone && (
              <span className="flex items-center gap-1.5">
