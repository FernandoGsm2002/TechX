...
import {
  faShieldHalved, faShieldCat, faMagnifyingGlass, faClock,
  faTicket, faArrowLeft, faLockOpen, faTriangleExclamation,
  faChevronRight, faArrowsRotate, faBan, faClockRotateLeft, faMobileScreen,
  faPhone, faUser, faCircleCheck, faCircleXmark, faCircleInfo,
...
function StatCard({ label, value, icon, colorClass, loading }: {
  label: string; value: number; icon: IconDefinition;
  colorClass: string; loading: boolean;
}) {
  const textColor = colorClass.split(' ').find(c => c.startsWith('text-')) || 'text-muted-foreground';
  return (
    <div className="rounded-xl border border-border/50 bg-card/60 p-3.5 space-y-1 overflow-hidden backdrop-blur">
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2 min-w-0">
            <div className={`flex size-8 items-center justify-center rounded-lg shrink-0 ${
              ...
              {g.source === "ticket"
                ? <FaIcon icon={faTicket} size={14} />
                : <FaIcon icon={faLockOpen} size={14} />}
            </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-muted-foreground">
          {imei && (
            <span className="flex items-center gap-1 col-span-2 font-mono text-foreground/70">
              <FaIcon icon={faMobileScreen} size={11} className="shrink-0" /> {imei}
            </span>
          )}
          {phone && (
            <span className="flex items-center gap-1">
              <FaIcon icon={faPhone} size={11} className="shrink-0" /> {phone}
            </span>
          )}
          <span className="flex items-center gap-1">
            <FaIcon icon={faClock} size={11} className="shrink-0" />
            {format(parseISO(g.end_date), "dd MMM yyyy", { locale: es })}
          </span>
          {g.claim_count > 0 && (
            <span className="flex items-center gap-1 text-amber-400">
              <FaIcon icon={faClockRotateLeft} size={11} className="shrink-0" /> {g.claim_count} reclamo{g.claim_count !== 1 ? "s" : ""}
            </span>
          )}
          <span className={`flex items-center gap-1 font-medium ${
            ...
            <FaIcon icon={faShieldHalved} size={11} className="shrink-0" />
          ...
          {linkHref && (
            <Link href={linkHref}>
              <Button variant="outline" size="sm" className="h-7 text-xs gap-1">
                <FaIcon icon={faTicket} size={11} /> Ver ticket
                <FaIcon icon={faChevronRight} size={11} />
              </Button>
            </Link>
          )}
          {g.is_valid && (
            <Button
              ...
              onClick={() => onClaim(g)}
            >
              <FaIcon icon={faShieldCat} size={11} /> Reclamar garantia
            </Button>
          )}
          {isAdmin && g.is_active && (
            <Button
              ...
              onClick={() => onVoid(g)}
            >
              <FaIcon icon={faBan} size={11} /> Anular
            </Button>
          )}
          {isAdmin && !g.is_active && (
            <Button
            ...
            >
              <FaIcon icon={faArrowsRotate} size={11} /> Reactivar
            </Button>
          )}
          {/* Toggle cobros */}
          {g.is_active && (
    ...
    <div className="space-y-3">
      <div className="flex gap-2">
        ...
        <Button size="sm" variant="outline" onClick={() => setQuery(imei.trim())}
          ...
          <FaIcon icon={faMagnifyingGlass} size={13} /> Buscar
        </Button>
      </div>
          ...
          <div className="flex flex-col gap-1 items-end">
            ...
            {g.is_valid && (
              <Button size="sm" className="h-6 text-[11px] bg-blue-600 hover:bg-blue-700 gap-1"
                onClick={() => onSelect(g)}>
                <FaIcon icon={faShieldCat} size={11} /> Reclamar
              </Button>
            )}
          </div>
        </div>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <FaIcon icon={faShieldCat} size={14} className="text-blue-400" />
            Reclamar Garantia
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 py-1">
          ...
          <div className="bg-muted/30 rounded-lg p-3 space-y-1">
            ...
            <p className="text-xs text-emerald-400 flex items-center gap-1">
              <FaIcon icon={faShieldHalved} size={11} className="shrink-0" /> {daysLabel(garantia.days_remaining)}
            </p>
          </div>

          {/* Info importante */}
          <div className="flex gap-2 bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 text-xs text-blue-300">
            <FaIcon icon={faCircleInfo} size={13} className="shrink-0 mt-0.5" />
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          <AlertDialogTitle className="flex items-center gap-2">
            <FaIcon icon={faBan} size={14} className="text-destructive" /> Anular Garantia
          </AlertDialogTitle>
...
export default function GarantiasPage() {
  ...
  const TAB_CONFIG: { value: FilterTab; label: string; icon: IconDefinition }[] = [
    { value: "active",  label: "Vigentes",  icon: faShieldHalved },
    { value: "expired", label: "Vencidas",  icon: faShieldCat },
    { value: "voided",  label: "Anuladas",  icon: faBan },
    { value: "all",     label: "Todas",     icon: faClockRotateLeft },
  ];
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-xl font-bold flex items-center gap-2">
            <FaIcon icon={faShieldHalved} size={18} className="text-emerald-400" /> Garantias
          </h1>
        ...
        >
          <FaIcon icon={faMagnifyingGlass} size={13} />
        ...
        <Card className="border-blue-500/30 bg-blue-500/5">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <FaIcon icon={faMobileScreen} size={14} className="text-blue-400" />
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <StatCard label="Vigentes"    value={stats?.vigentes  ?? 0} icon={faShieldHalved}        colorClass="bg-emerald-500/20 text-emerald-400" loading={loadingStats} />
        <StatCard label="Por vencer"  value={stats?.porVencer ?? 0} icon={faTriangleExclamation} colorClass="bg-amber-500/20 text-amber-400"   loading={loadingStats} />
        <StatCard label="Vencidas"    value={stats?.vencidas  ?? 0} icon={faShieldCat}            colorClass="bg-red-500/20 text-red-400"       loading={loadingStats} />
        <StatCard label="Anuladas"    value={stats?.anuladas  ?? 0} icon={faBan}                  colorClass="bg-muted text-muted-foreground"   loading={loadingStats} />
        <StatCard label="Total"       value={stats?.total     ?? 0} icon={faClockRotateLeft}       colorClass="bg-blue-500/20 text-blue-400"     loading={loadingStats} />
        <StatCard label="Reclamos"    value={stats?.reclamos  ?? 0} icon={faShieldCat}             colorClass="bg-violet-500/20 text-violet-400" loading={loadingStats} />
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-muted-foreground">
          <FaIcon icon={faShieldHalved} size={42} className="opacity-20" />
          <p className="text-sm">
