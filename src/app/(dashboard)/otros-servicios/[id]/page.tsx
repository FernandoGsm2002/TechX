...
import {
  ArrowLeft, Phone, Smartphone, Tag, User, ShieldCheck,
  ShieldAlert, ShieldX, DollarSign, Clock, Banknote,
  Building2, CreditCard, Ghost, Hash, Info, Lock,
} from "lucide-react";
...
export default function OtroServicioDetailPage() {
  ...
  const customer     = (servicio as any).customers as { full_name?: string; phone?: string } | null;
  const techName     = ((servicio as any).assigned_profile as { full_name?: string } | null)?.full_name;
    ...
    <div className="space-y-4 pb-8">
        ...
        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${
          ...
          <Link href="/garantias">
            <Button variant="outline" size="sm" className="h-7 text-xs gap-1 shrink-0">
              <ShieldCheck className="size-3" /> Ver garantías
            </Button>
          </Link>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <>
                ...
                {(servicio.guest_phone as string | null) && (
                  <p className="flex items-center gap-1.5 text-muted-foreground">
                    <Phone className="size-3.5" /> {servicio.guest_phone as string}
                  </p>
                )}
              </>
            ) : (
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <Ghost className="size-4 text-violet-400" /> Detalle del servicio
            </CardTitle>
          </CardHeader>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <DollarSign className="size-4 text-muted-foreground" /> Pago
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1.5 text-sm">
            ...
            {(servicio as any).payment_method && (
              <div className="flex items-center justify-between">
                <span className="text-muted-foreground">Método</span>
                <span className="capitalize">{(servicio as any).payment_method}</span>
              </div>
            )}
            {(servicio as any).warranty_notes && (
              <div className="flex items-start gap-1.5 text-muted-foreground pt-1">
                ...
                <span className="text-xs">{(servicio as any).warranty_notes}</span>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        {/* ── Notas internas ── */}
        {(servicio as any).internal_notes && (
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <CardTitle className="text-sm flex items-center gap-2">
                <Info className="size-4 text-muted-foreground" /> Notas internas
              </CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <Card className="border-border/50 md:col-span-2">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm flex items-center gap-2">
              <Clock className="size-4 text-muted-foreground" /> Historial de cambios
            </CardTitle>
          </CardHeader>
